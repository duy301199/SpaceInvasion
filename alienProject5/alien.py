...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'images/explode_{n}.png') for n in range(4)]

    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/alien{m}{n}.png'),0,1.5) for n in range (2) ]for m in range(3)]

    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'images/my_ufo{n}.png'),0,1) for n in range(5)]

    alien_images.append(ufo_imgs)
    # credit = randint(50,200)
...
class Alien(Sprite):
    def __init__(self, game, sound, image_list, alien_index, start_index=0, ul=(0, 100), v=Vector(1, 0),
                 points=1211):
        super().__init__()
        self.game = game
        ...
        self.image = pg.image.load('images/alien00.png')
        self.screen_rect = self.screen.get_rect()
